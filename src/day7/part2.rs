...
fn main() {
    ...
    let space_left =
        total_space - total_size("/".to_string(), &dir_index, &file_index, &self_sizes);
    let to_free_up = required_space - space_left;
    ...
    let space_to_free_up = dirs
        .iter()
        .find(|folder_size| folder_size > &&to_free_up)
        
        .unwrap();

    dbg!(space_to_free_up);
